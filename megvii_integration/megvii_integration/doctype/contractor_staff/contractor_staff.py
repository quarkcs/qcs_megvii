...
class ContractorStaff(Document):
	def validate(self):
		contractor = frappe.get_doc("Contractors", self.company)
		device = frappe.get_doc("Megvii Devices", contractor.device)
		...
		if data_dump['status'] == 200:
			...
			url = device.device_url + "/api/passes/query"
			...
			data_dump = json.loads(response.text)
			frappe.errprint(response.text)
			#self.device_staff_id = data_dump['id']
